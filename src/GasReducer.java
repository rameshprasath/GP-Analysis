...
public class GasReducer extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context)
		...
		try
		{
			//Value format -> StateCode_MinPrice_MaxPrice (or) StateCode_MinPrice_MaxPrice;MaxPrice State
			for (Text value : values) {
				info = value.toString().split("_");

				//If combiner ran, output will have MaxPrice state as well
				if (info[2].indexOf(";")>0)
				{
					tempMaxState = info[2].substring(info[2].indexOf(";") + 1);
				...
				if (Float.parseFloat(info[2]) > maxPrice)
				{
					...
					maxPriceState = (tempMaxState != "")? tempMaxState: info[0];
				}
